...
const categoryImageMap: Record<string, string> = {
  "Beauty & Personal Care": "/category-images/beauty-personal-care.png",
  "Corporate & Promotional Gifts": "/category-images/corporate-promotional-gifts.png",
  "Books": "/category-images/books.png",
  "Books & Stationery": "/category-images/books.png",
  "Massager": "/category-images/massager.png",
...
/**
 * Amazon imports sometimes create one catalog card per pack/size (and, in a
 * few cases, per colour). Keep one sellable product and express only the
 * customer-facing size/quantity choices as variants. Colour is deliberately
 * not used as a selector for these families.
 */
export function consolidateProductFamilies(products: Product[]): Product[] {
  ...
  const replaceFamily = (
    primaryId: string,
    memberIds: string[],
    build: (primary: Product) => Product,
  ) => {
    const primary = byId.get(primaryId) || memberIds.map((id) => byId.get(id)).find(Boolean);
    if (!primary) return;
    memberIds.forEach((id) => {
      if (id !== primary.id) removed.add(id);
    });
    replacements.set(primary.id, build(primary));
  };
  ...
  replaceFamily(
    "prod_amz_B0FNN49PMX",
    [
      "prod_amz_B0FNN2D5CD", "prod_amz_B0FNN4WQNQ", "prod_amz_B0FNN66P5B",
      "prod_amz_B0FNN49PMX", "prod_amz_B0FNN6BRJD", "prod_amz_B0FNN7W66B",
      "prod_amz_B0FNN7Q41H", "prod_amz_B0FNN672TC",
    ],
    (product) => ({
      ...product,
      name: "400ml Glass Water Bottle with Silicone Protective Sleeve",
  ...
  return products
    ...
    .map((product) => replacements.get(product.id) || product);
}

export const initialProducts: Product[] = consolidateProductFamilies(rawInitialProducts);
    ...
    if (stored) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          setProducts(consolidateProductFamilies(parsed).map((p: any) => normalizeProduct({
            ...p,
    ...
    fetch("/api/products", { cache: "no-store" })
      ...
      .then((globalList) => {
        if (Array.isArray(globalList) && globalList.length > 0) {
          const normalizedGlobal = consolidateProductFamilies(globalList).map((p: Product) => normalizeProduct(p));
